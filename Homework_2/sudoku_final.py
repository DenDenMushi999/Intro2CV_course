...
def find_cells_digits(cells, debug=False):
    ...
    for i in range(9):
        for j in range(9):
            ...
            open_iters = 2
            thresh = cv.morphologyEx(thresh, cv.MORPH_OPEN, open_kernel, iterations=open_iters, borderType=cv.BORDER_CONSTANT)
            # thresh = cv.morphologyEx(thresh, cv.MORPH_CLOSE, open_kernel, iterations=open_iters, borderType=cv.BORDER_CONSTANT)
            # thresh = cv.morphologyEx(thresh, cv.MORPH_DILATE, open_kernel, iterations=1, borderType=cv.BORDER_CONSTANT)
            thresh[:,:10] = 0
            thresh[:,-10:] = 0
            thresh[:10,:] = 0
            thresh[-10:,:] = 0
            ...
            if debug:
                ...
                det_img = np.zeros((*thresh.shape,3))
                det_img[:,:,0] = thresh
                det_img[:,:,1] = thresh
                det_img[:,:,2] = thresh
                det_img = cv.drawContours(det_img, contours, -1, color=(255,255,0), thickness=3)
            ...
            if len(contours):
                ...
                for (c, ar) in zip(contours, areas):
                    if ar < max_area_thr and ar > cont_area_thr:
                        dig_contours.append(c)
                        dig_areas.append(ar)
                if len(dig_areas):
                    max_ind = np.argmax(dig_areas)
                    area = dig_areas[max_ind]
                    dig_cnt = dig_contours[max_ind]
                else:
                    digit_cells[i,j] = -1
            else:
                digit_cells[i,j] = -1
            ...
            cell = cv.GaussianBlur(cell, (5, 5), 3)
...
def find_thresh_for_digits(img):
    ...
    open_iters = 2
    thresh = cv.morphologyEx(thresh, cv.MORPH_OPEN, open_kernel, iterations=open_iters, borderType=cv.BORDER_CONSTANT)
    thresh = cv.morphologyEx(thresh, cv.MORPH_DILATE, open_kernel, iterations=open_iters, borderType=cv.BORDER_CONSTANT)
...
if __name__ == '__main__':
    ...
    debug=True
    img = cv.imread(TRAIN_IMG_NAMES[5])
