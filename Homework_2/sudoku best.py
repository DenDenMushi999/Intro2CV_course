...
class SudokuHandler:
    ...
    def find_puzzle_mask( self, image: np.ndarray, debug: bool = False ):
        area_thr = 700*700
    ...
    def warp_puzzles( self, puzzle_contours, img_orig, debug=False):
    ...
    def warp_puzzles_old( self, puzzle_contours, img_orig, debug=False):
    ...
    def find_puzzle( self, img, debug=False ):
        puzzle_mask, puzzle_cnts = self.find_puzzle_mask(img, debug=False)
        # return puzzle_mask, warp_puzzles_old( puzzle_cnts, img, debug=True)
        return puzzle_mask, self.warp_puzzles_old( puzzle_cnts, img, debug=debug)


    def find_cells(self, img, debug=False):
        """
    ...
    def find_cells_manual( self, warped_gray, debug=False ):
    ...
    def find_cells_digits(self, cells, debug=False):
    ...
    def predict_image(self, image: np.ndarray, debug=False) -> (np.ndarray, list):

        mask, warp_puzzles = self.find_puzzle(image, debug=debug)

        sudoku_digits = []
        for warp in warp_puzzles:
            puzzle_cells = self.find_cells_manual(cv.cvtColor(warp, cv.COLOR_BGR2GRAY))
            digit_cells, pred_vals = self.find_cells_digits(puzzle_cells, debug=debug)
            sudoku_digits.append(digit_cells)
        return mask, sudoku_digits

    def find_thresh_for_digits(self, img):
...
if __name__ == '__main__':
    ...
    task_solver = SudokuHandler()
    mask, sudoku_digits = task_solver.predict_image(img, debug=False)
