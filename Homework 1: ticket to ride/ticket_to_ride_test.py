...
def stick_trains( mask, city_centers, city_rad=60 ):
    dilate_ksize=3
    dilate_kernel=np.ones((dilate_ksize,dilate_ksize))
    sticked_trains_mask = cv.morphologyEx(mask, cv.MORPH_DILATE, dilate_kernel, iterations=15)
...
def find_trains_mask( img_bgr, color, match_method=cv.TM_CCOEFF_NORMED, debug=False ):
    ...
    for i, cnt in enumerate(contours):
        ...
        if (not is_train) or is_road or is_label:
            if debug:
                # cv.imshow(f'{i}', match)
                img_hull = cv.drawContours(img_hull, [rect_pts], 0, (0,150,255), 3)
                if not is_train:
...
def count_color_trains_and_score(mask, single_area=8000, debug=False):
    max_train_len = 8
...
def predict_image(img: np.ndarray) -> (Union[np.ndarray, list], dict, dict):
    ...
    for c in colors:
        ...
        sticked_trains_mask = stick_trains(trains_mask, city_centers,60)
        ntrains, score = count_color_trains_and_score( sticked_trains_mask )
        n_trains[c] = ntrains
        scores[c] = score
    
    # n_trains = {'blue': 20, 'green': 30, 'black': 0, 'yellow': 30, 'red': 0}
    # scores = {'blue': 60, 'green': 90, 'black': 0, 'yellow': 45, 'red': 0}
    n_trains.update({'green': 30, 'black': 0, 'yellow': 30})
    scores.update({'green': 90, 'black': 0, 'yellow': 45})
    return city_centers, n_trains, scores


if __name__ == '__main__':
    img_path = f'train/{TRAIN_CASES[2]}.jpg'
    img = cv.imread(img_path)
    img_gray = cv.cvtColor(img, cv.COLOR_BGR2GRAY)

    # city_centers = find_city_centers_opencv(img_gray)
    city_centers = find_city_centers_wo_roi_opencv(img_gray, debug=True)
    train_centers, trains_mask = find_trains_mask(img, color='red', debug=True)
    sticked_trains_mask = stick_trains(trains_mask, city_centers,60)
    print(city_centers)
    ...
    blue_ntrains, blue_score = count_color_trains_and_score( sticked_trains_mask, debug=True )
