...
class TicketToRideHandler:
    ...
    def find_city_centers_wo_roi_opencv(self, img_gray, method=cv.TM_CCOEFF_NORMED, debug=False):
        if not debug:
            ...
            city_detector = CityDetector()
            win_name = f'tune_CityDetector.find_city_centers_opencv'
            cv.namedWindow(win_name)
            var_names = ['threshold, woi_win']
            val_coeffs = [100, 1]
            max_vals = [100, 200]

            for var, coef, max_val in zip(var_names, val_coeffs, max_vals):
                cv.createTrackbar(var, win_name, getattr(city_detector, var*coef, max_val, empty))

            while True:
                for var in var_names:
                    if var in dir(city_detector):
                        setattr(city_detector, var, cv.getTrackbarPos(var, win_name))
                    else :
                        raise AttributeError(f'object \'city_detector\' has no attribute \'{var}\'')
    ...
    def find_one_color_trains_mask(self, img_bgr, color, match_method=cv.TM_CCOEFF_NORMED, debug=False):
        ...
        return train_detector.find_one_color_trains_mask(img_bgr, match_method=cv.TM_CCOEFF_NORMED, debug=debug)
    ...
    def find_city_centers_wo_roi_opencv_test(self, img_gray, method=cv.TM_CCOEFF_NORMED):
        threshold = 0.58
        cv.namedWindow('find_thr')
        cv.createTrackbar("match_threshold", "find_thr", int(threshold*100), 100, empty)
        match_centers = []
        w_templ = CITY_TEMPL.shape[1]
        h_templ = CITY_TEMPL.shape[0]
        match = cv.matchTemplate(img_gray, CITY_TEMPL_GRAY, method=method )
        ...
        while True:
            threshold = cv.getTrackbarPos("match_threshold", "find_thr")/100
    ...
    def find_one_color_trains_mask_test( self, img_bgr, color, match_method=cv.TM_CCOEFF_NORMED, debug=False ):
        assert color in COLORS
        ...
        mask = self.color_filter(img, color)
    ...
    def count_color_trains_and_score_test(self, mask, color):
        max_train_len = 8
        ...
        cv.namedWindow('find_trains_len_thr')
        cv.createTrackbar("len_thr", 'find_trains_len_thr', single_len, 3000, empty)

        while True:
            single_len = cv.getTrackbarPos("len_thr", "find_trains_len_thr")
            max_train_len = single_len*max_train_len
            contours, hierarchy = cv.findContours(mask, cv.RETR_TREE, cv.CHAIN_APPROX_SIMPLE)
            n_trains = [None]*len(contours)
            # lens = [cv.contourlen(cnt) for cnt in contours]
            # score
...
if __name__ == '__main__':
    img_path = f'train/{TRAIN_CASES[4]}.jpg'
    ...
    # find_city_centers_wo_roi_opencv_test(img_gray)

    task_handler = TicketToRideHandler()
    city_centers = task_handler.find_city_centers_wo_roi_opencv(img_gray, debug=True)
    train_centers, trains_mask = task_handler.find_one_color_trains_mask(img, color=color, debug=True)
    ...
    blue_ntrains, blue_score = task_handler.count_color_trains_and_score_test( sticked_trains_mask, color=color)
