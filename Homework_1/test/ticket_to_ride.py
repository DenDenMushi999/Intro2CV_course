...
PATH = '/home/dendav/projects/Intro2CV_course/Homework_1/test'
...
APPROX_CENTERS = load_json(f'{PATH}/all_centers.json')

# IMG = cv.imread(f'train/{TRAIN_CASES[2]}.jpg')
# IMG = cv.cvtColor(IMG, cv.COLOR_BGR2RGB)

# CITY_TEMPL = IMG[197:260, 622:685]
CITY_TEMPL = cv.imread(f'{PATH}/city_template.jpg')
CITY_TEMPL_GRAY = cv.cvtColor(CITY_TEMPL, cv.COLOR_BGR2GRAY)

TRAIN_TEMPLATES = { c : cv.imread(f'{PATH}/{c}_template.jpg') for c in COLORS }
...
if __name__ == '__main__':
    ...
    img_path = f'{PATH}/{TRAIN_CASES[0]}.jpg'
    print(img_path)
    img = cv.imread(img_path)
    img_gray = cv.cvtColor(img, cv.COLOR_BGR2GRAY)
    city_centers, n_trains, scores = predict_image(img)
    print('end result: ')
    print(city_centers)
    print(n_trains)
    print(scores)
